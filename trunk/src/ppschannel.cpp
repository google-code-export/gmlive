...
PPSChannel::PPSChannel(MainWindow* parent_):Channel(parent_)
					    ,wget_pid(-1)
					    ,refresh(false)
{
	init();
...
bool PPSChannel::init()
{
	...
	if(!file){
		snprintf(buf,512,"%s/pps.lst",DATA_DIR);
		file.open(buf);
	}
	std::string line;
	std::string name;
...
void PPSChannel::addLine(int num, const Glib::ustring & name,const std::string& stream,const Glib::ustring& groupname)
{
...
void PPSChannel::refresh_list()
{
	...
	if (pid == 0) {
		...
		const char* argv[6];
		argv[0] = "wget";
		//argv[1] = "http://www.gooth.cn/pps.lst";
